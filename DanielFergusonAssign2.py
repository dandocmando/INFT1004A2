...
Auth ID: 3374690
Date: Start -> 10/04/22 Functionality Complete -> 12/04/22 Upload -> 20/05/22
...
class MainMenu:
    ...
    def menu(self):
        ...
        if choice == '5':
            clean_system()
        if choice.lower() == 'q':
            clean_system()  # remove this if you would like the program to retain CSV data
            print("Goodbye! ")
    ...
    def spending_spree(self):  # 2
        ...
        # export_df['EndingBalance'] = 0  # creates a new column with the name EndingBalance
        export_df.loc[0, 'EndingBalance'] = gc_max_spend - sum(gc_cost_lst)  # assigns ending balance
        export_df.loc[0, 'CurrentBalance'] = gc_max_spend  # this seems redundant, but it has been added
        export_df = export_df.loc[:,
                    ['GiftCardName', 'ItemDescription', 'ItemPrice', 'CurrentBalance', 'EndingBalance']]
    ...
    def gc_history(self):  # 4
        ...
        sl(self.dt)
        print('')
        print("Current (Original) Balance: $" + str(gc_indiv_purchases.loc[0, "CurrentBalance"]))  # max spend gc value
        sl(self.dt)
        print("Ending Balance: $" + str(gc_indiv_purchases.loc[0, 'EndingBalance']))  # prints out the unspent gc value
        print('')
        sl(self.dt)

        re_run = input("Would you like to view another gift card? (Yes or No): ")
        if re_run.upper() == "Y" or re_run.upper() == "YES":
            print('')
            self.gc_history()  # repeats
        else:
            self.menu()  # back to menu
    ...
    def create_initial_csv(self):
        ...
        spending_history = pd.DataFrame(
            columns=['GiftCardName', 'ItemDescription', 'ItemPrice', 'CurrentBalance', 'EndingBalance'])
    ...
    def menu_relaunch(self):
        # This runs at the bottom of most parts of the program to let the user return to the menu
        ...
        if re_to_menu.upper() == "Y" or re_to_menu.upper() == "YES":
            ...
            clean_system()  # remove if you want to retain CSV data
